...
{
    public static Dictionary<string, string[]> ValidateAddSoftwareDto(FeedbackApi.PostFeedbackDto feedback)
    {
        var errors = new Dictionary<string, string[]>();

        if (string.IsNullOrWhiteSpace(feedback.CourseCode))
        {
            errors[nameof(feedback.CourseCode)] = ["Course Code is required"];
        }
        if (feedback.Helpful < 1 || feedback.Helpful > 10)
        {
            errors[nameof(feedback.CourseCode)] = ["Helpful must be between 1 and 10"];
        }

        if (feedback.Satisfied < 1 || feedback.Satisfied > 10)
        {
            errors[nameof(feedback.CourseCode)] = ["Satisfied must be between 1 and 10"];
        }

        if (feedback.Knowledgeable < 1 || feedback.Knowledgeable > 10)
        {
            errors[nameof(feedback.CourseCode)] = ["Knowledgeable must be between 1 and 10"];
        }
